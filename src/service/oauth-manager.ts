...
class OAuthManager {
    ...
    async getTokenFromCode(code: string) {
        ...
        const response = await axiosInstance.post(this.tokenEndpoint, new URLSearchParams(data), {
            headers: {
    ...
    checkCredentials() {
        return Cookies.get('oauth_access_token') || Cookies.get('oauth_refresh_token');
    }
    ...
    saveTokens(response: any) {
        ...
        accessTokenExpiry.setSeconds(accessTokenExpiry.getSeconds() + response.data.expires_in);
    ...
    async me() {
        ...
        const response = await axiosInstance.get(endpoint, { headers: { 'Authorization': `Bearer ${token}` } });
